...
GYRO_ALPHA_LEFT_LIMIT = 30.0   # The positive Alpha value for max left turn (0 to 30)
GYRO_ALPHA_RIGHT_LIMIT = 30.0  # The amount of Alpha decrease for max right turn (360 to 330)
...
async def ws_handler(websocket):
    ...
    try:
        async for message in websocket:
            ...
            # User requested Alpha only mapped to X axis
            # Ensure alpha wraps correctly between 0 and 360, especially if calibration offsets are applied
            alpha = alpha % 360.0
            ...
            if 0 <= alpha <= 180:
                ...
                # Map 360 to Right Limit -> 0 to 1.0
    ...
    try:
        while True:
            ...
            if current_time - last_ui_update > 0.033:  # Render at ~30 FPS
                ...
                limits_text = font.render(f"Limits (L/R): {GYRO_ALPHA_LEFT_LIMIT:.1f} / {GYRO_ALPHA_RIGHT_LIMIT:.1f}", True, (255, 255, 100))
